...
import { readCachedProspect } from "@/lib/data/cache";
...
export default function ProspectDetailPage() {
  ...
  const [p, setP] = useState<Prospect | null | undefined>(undefined);
  const [showToast, setShowToast] = useState(false);

  useEffect(() => {
    const curated = findProspect(id);
    if (curated) {
      setP(curated);
      return;
    }
    setP(readCachedProspect(id));
  }, [id]);

  if (p === undefined) {
    return <div className="px-6 py-12 text-center text-mute text-sm">Loading…</div>;
  }
  ...
  if (!p) {
    return (
      <div className="px-6 py-12 text-center">
        ...
        <p className="text-mute text-[11px] mt-1">If you got here from a fresh tab, the crawl cache won&apos;t carry over. Re-run the trip from <Link href="/plan" className="text-accent font-semibold">Plan</Link>.</p>
  ...
  const tripLabel = area?.name ? `${area.name} Trip` : "Trip";
  ...
  return (
    <div className="relative">
      <div className="px-6 pt-2 flex justify-between text-[13px] text-accent font-semibold">
        <button onClick={() => router.back()} className="active:translate-x-[-2px] transition-transform">‹ {tripLabel}</button>
      ...
      <div className="px-6 pt-3.5 pb-1.5">
        ...
        ">
          {area?.name ?? p.area}{p.distanceMi !== undefined ? ` · ${p.distanceMi}mi` : ""}
        </span>
        <h3
        ...
        <div className="text-[12px] text-mute mt-1">
          {p.industry}
          {p.founded ? ` · est. ${p.founded} · ${yearsSince(p.founded)} yrs` : null}
          {p.revenueEstimate ? ` · ${formatRevenueBand(p.revenueEstimate)}` : ""}
        </div>
        {(p.address || p.phone || p.website) && (
          <div className="text-[11px] text-mute mt-1.5 space-y-0.5">
            {p.address && <div>{p.address}</div>}
            {p.phone && <div>{p.phone}</div>}
            {p.website && (
              <a href={p.website} target="_blank" rel="noreferrer" className="text-accent font-semibold inline-block">
        ...
        {p.provisional && (
          <div className="mt-2 inline-block text-[10px] bg-black/[.06] text-mute px-2 py-1 rounded font-semibold tracking-[.04em] uppercase">
            Provisional · v2 enrichment pending
          </div>
        )}
      </div>
